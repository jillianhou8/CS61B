...
class GUI extends TopLevel implements View, Reporter {
     ...
     * The board widget.
     */
    private Pattern manual_pattern = Pattern.compile("(black|white)$?");
    /**
     * The board widget.
     */
    private Pattern auto_pattern = Pattern.compile("(black|white)$?");
    ...
    private void manual(String dummy) {
        String reply =
                getTextInput("Timmy or YG?",
                        "Manual Player", "plain", "");
        if (reply != null) {
            Matcher matchy = manual_pattern.matcher(reply);
            if (matchy.matches()) {
                _pendingCommands.offer(String.format("manual %s",
                        matchy.group(1)));
            } else {
                showMessage("Timmy or YG?", "Error",
                        "error");
            }
        }
    }

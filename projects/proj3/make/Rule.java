...
class Rule {
    ...
    void addCommands(List<String> commands) {
        if (!_commands.isEmpty() && !commands.isEmpty()) {
            throw new IllegalStateException("Command set is not empty");
        }
    ...
    private void checkFinishedDependencies() {
        int v = getVertex();
        for (int dep : _depends.successors(v)) {
            Rule ruley = _depends.getLabel(dep);
            if (ruley.isUnfinished()) {
                error("error: Dependencies were not built before rebuilding");
            }
        }
    }
    ...
    void rebuild() {
        ...
        if (outOfDate()) {
            if (_commands.isEmpty()) {
                error("error: needs to be rebuilt, but has no commands");
            }

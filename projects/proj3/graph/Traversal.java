...
public abstract class Traversal {
    ...
    public void traverse(Collection<Integer> V0) {
        ...
        while (!_fringe.isEmpty()) {
            ...
            if (!marked(curr)) {
                ...
                int index = _visited.indexOf(curr);
                if (shouldPostVisit(curr) && !_postvisited.get(index)) {
                    postVisit(curr);

                    _postvisited.set(index, true);
                }
            }
        }
    }
    ...
    /** For style bish. */
    private ArrayList<Boolean> _postvisited;
}

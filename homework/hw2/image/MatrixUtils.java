...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int each_row = 2; each_row < m.length; each_row += 1) {
            for (int entry = 0; entry < m[0].length; entry += 1) {
                double original = get(m, each_row, entry);
                double above = get(m, each_row - 1, entry);
                if (entry == 0) {
                    double above_right = get(m, each_row - 1, entry + 1);
                    am[each_row][entry] = original + java.lang.Math.min(above, above_right);
                }
                else if (entry == m[0].length - 1) {
                    double above_left = get(m, each_row - 1, entry - 1);
                    am[each_row][entry] = original + java.lang.Math.min(above, above_left);
                }
                double above_left = get(m, each_row - 1, entry - 1);
                double above_right = get(m, each_row - 1, entry + 1);
                am[each_row][entry] = original + java.lang.Math.min(java.lang.Math.min(above, above_left), above_right);
            }
        }
        return am;
    }
    ...
    /**  A helper function I wrote to retrieve the entry e[r][c] if r and c
     * are valid and e is not an empty array.
     */
    public static double get(double[][] e, int r, int c) {
        if (e != null && r <= e.length && c <= e[0].length) {
            return e[r][c];
        }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        ...
        return accumulateVertical(helper(m));
    }



    public static double[][] helper(double[][] m) {
